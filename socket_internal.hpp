    ...
    fd(-1)
{
  ...
  if (dst || src)
              ...
              if ((!dst &&
                    ...
                    (conret == -1 && errno == EINPROGRESS))))
                {
                  std::cout
                    ...
                    << " with fd " << std::to_string(fd) << std::endl;
                  finished = (conret != -1);
                  // Success.
                  return;
                }
    ...
    {
      throw std::invalid_argument("No source or destination given for socket");
    }
  throw(e);
}

template <typename T>
bool Connections<T>::SocketConnection::finish()
{
  int ret = connect(fd, reinterpret_cast<const struct sockaddr *>(&destination),
                    destination_len);
...
template <typename T>
bool Connections<T>::SocketConnection::read(std::stringstream &data) const
{
  char buf[BUFSIZ];
  int ret;
...
template <typename T>
bool Connections<T>::SocketConnection::write(const std::string &data) const
{
  ...
  if (ret != (int)data.length() &&
    ...
    {
      std::cerr << "Connection " << std::to_string(fd) << " write failed: "
...
template <typename T>
int Connections<T>::process(int timeout)
{
  int ret = -1;
